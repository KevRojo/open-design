...
The local test replays these bytes at the CLI runner boundary through the real adapter/service/SQLite. It does not run a live Go process, PostgreSQL or browser. Team-scope consumption is verified. Personal-scope anchorless tombstone handling is a separately recorded failing probe (publication filePath filtering), not covered by this passing team test.
